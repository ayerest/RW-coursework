...
let names: [String] = ["sam", "george", "fred", "cynthia"]

let namesTogether = names.reduce("") { result, name in
    return result + " " + name
}
...
let namesAndAges = ["George": 45, "Simon": 22, "Rachel": 31, "Theo": 9, "Roberta": 12]

let kids = namesAndAges.filter {
    $0.value < 18
}
